/*
* (C) Copyright 2009-2013 Manaty SARL (http://manaty.net/) and contributors.
*
* Licensed under the GNU Public Licence, Version 2.0 (the "License");
* you may not use this file except in compliance with the License.
* You may obtain a copy of the License at
*
*     http://www.gnu.org/licenses/gpl-2.0.txt
*
* Unless required by applicable law or agreed to in writing, software
* distributed under the License is distributed on an "AS IS" BASIS,
* WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
* See the License for the specific language governing permissions and
* limitations under the License.
*/
package org.grieg.communication;
import org.grieg.GriegConfig;
import org.grieg.communication.inputLoader.MessageInputHandler;
import org.grieg.communication.inputLoader.MessageInputLoader;
import org.grieg.communication.inputLoader.MessageValidator;
import org.grieg.communication.ticket.MessageTicket;
import org.meveo.config.MeveoConfig;
import org.meveo.config.MeveoFileConfig;
import org.meveo.config.task.MeveoTask;
import org.meveo.core.inputhandler.InputHandler;
import org.meveo.core.inputloader.InputLoader;
import org.meveo.core.launcher.TaskLauncher;
import org.meveo.core.outputhandler.NoOutputHandler;
import org.meveo.core.outputhandler.OutputHandler;
import org.meveo.core.outputproducer.NoOutputProducer;
import org.meveo.core.outputproducer.OutputProducer;
import org.meveo.core.process.Processor;
import org.meveo.core.process.step.AbstractProcessStep;
import org.meveo.core.validator.Validator;

import com.google.inject.AbstractModule;
import com.google.inject.TypeLiteral;

public class CommunicationModule extends AbstractModule {

	@Override
	protected void configure() {
        bind(TaskLauncher.class).to(MessageTaskLauncher.class);	
        bind(new TypeLiteral<MeveoTask<MessageTicket>>() {}).to(MessageConverterTask.class);	
        bind(MeveoConfig.class).to(GriegConfig.class);
        bind(MeveoFileConfig.class).to(GriegConfig.class);
        bind(GriegConfig.class).asEagerSingleton();
        bind(InputLoader.class).to(MessageInputLoader.class);
        bind(new TypeLiteral<InputHandler<MessageTicket>>() {}).to(MessageInputHandler.class);
        bind(new TypeLiteral<Processor<MessageTicket>>() {}).to(MessageProcessor.class);
        bind(new TypeLiteral<Validator<MessageTicket>>() {}).to(MessageValidator.class);
        bind(OutputProducer.class).to(NoOutputProducer.class);
        bind(OutputHandler.class).to(NoOutputHandler.class);
        bind(new TypeLiteral<AbstractProcessStep<MessageTicket>>() {}).toProvider(MessageStepProvider.class);
        
	}

}
