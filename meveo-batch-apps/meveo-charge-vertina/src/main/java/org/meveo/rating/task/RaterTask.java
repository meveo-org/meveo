...
/**
 * Rater task. Copy of rater task, just ticket type is not VertinaTicket but
 * RatingTicket. Should be temporary.
 * 
 * @author Ignas Lelys
 * @created Jan 4, 2011
 */
public class RaterTask extends MeveoTask<RatingTicket> {

    private static final Logger logger = Logger.getLogger(RatingTicket.class);

    @Inject
    public RaterTask(Provider<InputLoader> inputLoaderProvider,
            Provider<InputHandler<RatingTicket>> inputHandlerProvider, Provider<OutputHandler> outputHandlerProvider) {
