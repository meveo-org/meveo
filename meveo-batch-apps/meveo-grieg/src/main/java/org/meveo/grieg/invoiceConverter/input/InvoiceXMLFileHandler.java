 ...
 * 
 */
public class InvoiceXMLFileHandler extends AbstractFileInputHandler<GriegTicket> {

    private static final Logger logger = Logger.getLogger(InvoiceXMLFileHandler.class);
     ...
     */
    @Inject
    public InvoiceXMLFileHandler(Processor<GriegTicket> processor, OutputProducer outputProducer,
     ...
     */
    // TODO maybe better to deprecate this method and handle rejected and
    // ignored inputs in InputLoader. Set rejected and ignored objects to Input.
    @Override
    protected void afterProcessing(Input input, TaskExecution<GriegTicket> taskExecution) {
        ...
        if (taskExecution.getRejectedTicketsCount() > 0) {
            FileUtils.moveFile(((GriegConfig)config).getRejectedTicketsFilesDirectory(), rejectedTicketsFile,
