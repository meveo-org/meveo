...
public class ScriptInstanceService extends CustomScriptService<ScriptInstance> {
	...
	public List<CustomScript> getScriptInstancesWithError() {
		return getEntityManager().createNamedQuery("CustomScript.getScriptInstanceOnError", CustomScript.class).setParameter("isError", Boolean.TRUE).getResultList();
	}
	...
	protected void executeEngine(ScriptInterface engine, Map<String, Object> context) throws ScriptExecutionException {
		...
		try {
			txType = getEntityManager().createQuery("SELECT transactionType FROM ScriptInstance WHERE code = :code", ScriptTransactionType.class).setParameter("code", engine.getClass().getName()).getSingleResult();
	...
	public void postModuleInstall(@Observes @ModulePostInstall MeveoModule module) throws InvalidScriptException {
		var scripts = module.getModuleItems().stream().filter(item -> item.getItemClass().equals(ScriptInstance.class.getName())).map(item -> findByCode(item.getItemCode())).collect(Collectors.toList());
