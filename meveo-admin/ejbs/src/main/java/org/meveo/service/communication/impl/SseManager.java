...
/**
 * This class allow web client to subscribe to WebNotifications and publish messages
 */
@ApplicationScoped
public class SseManager {
    private class FilteringSink implements SseEventSink {
    ...
    @POST
    @Path("/sse/publish/{notif}")
    public void broadcast(@PathParam("notif") final String notif,String message,@Context HttpServletRequest requestContext,
                          @Context SecurityContext securityContext) throws IOException
    {
        ...
        log.debug("broadcast from "+callerIp+" username="+userName+" to notif:"+notif+" message:"+message);
        if (notif == null){
            throw new IllegalStateException("No web notification name set.");
        }
        WebNotification webNotification = webNotificationService.findByCode(notif);
        ...
        for (Map.Entry<String, FilteringSink> entry : filteringSinks.entrySet()) {
            ...
            if(filteringSink.isClosed()){
                ...
                if(filteringSink.getfilterEL()==null || filteringSink.getfilterEL().isEmpty()){
                    ...
                    try {
                        Object res = MeveoValueExpressionWrapper.evaluateExpression(filteringSink.getfilterEL(), context, Boolean.class);
                        boolean result = (Boolean) res;
                        if (result) {
                            listeningSinks.put(entry.getKey(),filteringSink);
                        }
                    } catch (Exception e) {
                        throw new IllegalStateException("Expression " + filteringSink.getfilterEL() + " do not evaluate to boolean");
                    }
                }
            }
        }
