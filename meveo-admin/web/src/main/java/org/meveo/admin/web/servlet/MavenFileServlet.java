 ...
 * @since 6.6.0
 */
@WebServlet(name = "mavenMavenFileServlet", urlPatterns = "/maven/*", loadOnStartup = 1000)
public class MavenFileServlet extends HttpServlet {

	// Constants
	// ----------------------------------------------------------------------------------
	private static final long serialVersionUID = 5387205359734273946L;
	...
	public void initUserPath() throws ServletException {
		...
		if (this.userPath == null) {
			throw new ServletException("MavenFileServlet init param 'userPath' is required.");

		} else {
			File path = new File(this.userPath);
			if (!path.exists()) {
				throw new ServletException("MavenFileServlet init param 'userPath' value '" + this.userPath + "' does actually not exist in file system.");

			} else if (!path.isDirectory()) {
				throw new ServletException("MavenFileServlet init param 'userPath' value '" + this.userPath + "' is actually not a directory in file system.");

			} else if (!path.canRead()) {
				throw new ServletException("MavenFileServlet init param 'userPath' value '" + this.userPath + "' is actually not readable in file system.");
			}
		}
	}
	 ...
	 * @throws ServletException if user path is not successfully initialize
	 */
	private void processRequest(HttpServletRequest request, HttpServletResponse response, boolean content) throws IOException, ServletException {
