...
@Install(precedence = APPLICATION, value = false)
@Scope(org.jboss.seam.ScopeType.APPLICATION)
@BypassInterceptors
public class CacheableJpaPermissionStore extends JpaPermissionStore {

    private static final long serialVersionUID = 1L;

    private static final LogProvider log = Logging.getLogProvider(CacheableJpaPermissionStore.class);
    ...
    /**
     * Creates a Query that returns a list of permission records for the
     * specified parameters.
     * 
     * @param target
     *            The target of the permission, may be null
     * @param targets
     *            A set of permission targets, may be null
     * @param recipient
     *            The permission recipient, may be null
    ...
    protected Query createPermissionQuery(Object target, Set targets, Principal recipient, Discrimination discrimination) {
        ...
        if (!queryCache.containsKey(queryKey)) {
            ...
            // If there is no discrimination, then don't add such a condition to
            // the query
    ...
    public boolean grantPermission(Permission permission) {
        return updatePermissionActions(permission.getTarget(), permission.getRecipient(), new String[] { permission
                .getAction() }, true);
    }
     ...
     * 
     * @param target
     *            The target object to update permissions for
     * @param recipient
     *            The recipient to update permissions for
     * @param actions
     *            The actions that will be updated
     * @param set
     *            true if the specified actions are to be granted, false if they
     *            are to be revoked
    ...
    protected boolean updatePermissionActions(Object target, Principal recipient, String[] actions, boolean set) {
        ...
        try {
            if (recipientIsRole) {
                if (rolePermissionClass != null) {
                    List permissions = createPermissionQuery(target, null, recipient, Discrimination.role)
                            .getResultList();
                    ...
                    ActionSet actionSet = metadata.createActionSet(target.getClass(), roleActionProperty.getValue(
                            instance).toString());
                    ...
                    if (permissions.size() > 1) {
                        // This is where it gets a little messy.. if there is
                        // more than one permission
                        // record, then we need to consolidate them all into
                        // just the first one
                    ...
                    if (!actionSet.isEmpty()) {
                        ...
                        // No actions remaining in set, so just remove the
                        // record
            ...
            List permissions = createPermissionQuery(target, null, recipient,
                    recipientIsRole ? Discrimination.role : Discrimination.user).getResultList();
            ...
            ActionSet actionSet = metadata.createActionSet(target.getClass(), actionProperty.getValue(instance)
                    .toString());
            ...
            if (permissions.size() > 1) {
                // Same as with roles, consolidate the records if there is more
                // than one
                for (Object p : permissions) {
    ...
    /**
     * Groups a list of arbitrary permissions into a more easily-consumed
     * structure
     * 
     * @param permissions
     *            The list of permissions to group
     * @return
     */
    ...
    /**
     * If the user or role properties in the entity class refer to other
     * entities, then this method uses the JpaIdentityStore (if available) to
     * lookup that user or role entity. Otherwise it simply returns the name of
     * the recipient.
     * 
     * @param recipient
    ...
    /**
     * Returns a list of all user and role permissions for the specified action
     * for all specified target objects
     */
    ...
    /**
     * Returns a list of all user and role permissions for a specific permission
     * target and action.
     */
    ...
    protected List<Permission> listPermissions(Object target, Set<Object> targets, String action) {
        ...
        for (Object permission : userPermissions) {
            ...
            if (targets != null) {
                ...
                if (target != null) {
                    actionSet = metadata.createActionSet(target.getClass(), actionProperty.getValue(permission)
                            .toString());
                }
            } else {
        ...
        if (rolePermissionClass != null) {
            ...
            for (Object permission : rolePermissions) {
                ...
                if (targets != null) {
                    ...
                    if (target != null) {
                        actionSet = metadata.createActionSet(target.getClass(), roleActionProperty.getValue(permission)
                                .toString());
                    }
                } else {
                    actionSet = metadata.createActionSet(target.getClass(), roleActionProperty.getValue(permission)
    ...
    private Principal lookupPrincipal(Map<String, Principal> cache, Object permission, boolean isUser) {
        Principal principal = resolvePrincipal(isUser ? userProperty.getValue(permission) : roleProperty
    ...
    public void clearPermissions(Object target) {
        ...
        // userPermissionClass.getName() + " p where p."
        // + targetProperty.getName() + " = :target");
        // q.setParameter("target", identifier);
        // q.executeUpdate();
        //
        // if (rolePermissionClass != null) {
        // em.createQuery(
        ...
        // + " = :target").setParameter("target", identifier).executeUpdate();
        // }
    }
}
